...
const Test = () => {
  ...
  const tracabilite = new web3.eth.Contract(CONTRACT_ABI_TRACABILITE, CONTRACT_ADDRESS_TRACABILITE);
    ...
    const setTransaction1 = async (t) => {
      t.preventDefault();
      // Get permission to access user funds to pay for gas fees
      const gas = await tracabilite.methods.seTtransactionFournisseurClient("did:ebsi:from","did:ebsi:to","2023-01-01",false).estimateGas();
      const post = await tracabilite.methods.seTtransactionFournisseurClient("did:ebsi:from","did:ebsi:to","2023-01-01",false).send({
        from: account,
        gas,
    ...
    const getTransaction1 = async (t) => {
      t.preventDefault();
      tracabilite.methods.getTransactionFournisseurClient().call((err, result) => {
        if (err){
           console.log(err)
        } else {
