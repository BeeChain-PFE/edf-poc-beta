...
contract TrustedIssuerList {
    ...
    mapping(ProposalType => uint256) neededAcceptanceRate;
    ...
    function createProposal(
        ...
        address _statekeeper,
        uint256 _newRate,
        ProposalType _rateType
    ) public onlyStatekeepers {
    ...
    function vote(uint256 _proposalId, bool _yea)
        public
        onlyStatekeepers
        notVotedYet(_proposalId)
    {
        require(
            proposals[_proposalId].state == State.IN_PROGRESS,
            "Proposal was already voted on."
        );
        if (_yea) {
    ...
    function enforceProposal(uint256 _proposalId)
        public
        onlyStatekeepers
        neededVotesReached(_proposalId)
    {
        require(
            proposals[_proposalId].state == State.IN_PROGRESS,
            "Proposal was already enforced."
        );
        uint256 needed = getNeededVotes(_proposalId);
        ...
        if (
            proposals[_proposalId].proposalType == ProposalType.ADD_STATEKEEPER
        ) {
        ...
        if (
            ...
            ProposalType.REMOVE_STATEKEEPER
        ) {
            proposals[_proposalId].state = State.ACCEPTED;
        ...
        if (
            proposals[_proposalId].proposalType ==
            ProposalType.CHANGE_ACCEPTANCE_RATE
        ) {
            proposals[_proposalId].state = State.ACCEPTED;
            neededAcceptanceRate[proposals[_proposalId].rateType] = proposals[
                _proposalId
            ].rate;
        }
    ...
    function addTrustedIssuer(string memory _issuerDID)
        public
        onlyStatekeepers
    {
    ...
    function removeTrustedIssuer(string memory _issuerDID)
        public
        onlyStatekeepers
    {
        if (trustedIssuers.map[_issuerDID]) {
        ...
        for (uint256 i = 0; i < trustedIssuers.keys.length; i++) {
            if (
                keccak256(abi.encodePacked(trustedIssuers.keys[i])) ==
        ...
        if (index > -1) {
            // Move to be delete element to last position and then pop it out -> this prevents "gaps" in the array
            trustedIssuers.keys[uint256(index)] = trustedIssuers.keys[
                trustedIssuers.keys.length - 1
            ];
    ...
    function isTrustedIssuer(string memory _issuerDID)
        public
        view
        returns (bool)
    {
    ...
    function getNeededVotes(uint256 _proposalId)
        public
        view
        returns (uint256 neededVotes)
    {
        return ((statekeepers.keys.length *
            neededAcceptanceRate[proposals[_proposalId].proposalType]) / 100);
    }
    ...
    modifier notVotedYet(uint256 _proposalId) {
        bool hasVoted = false;
        for (uint256 i = 0; i < proposals[_proposalId].yea.length; i++) {
            if (proposals[_proposalId].yea[i] == msg.sender) hasVoted = true;
        }
        for (uint256 i = 0; i < proposals[_proposalId].nay.length; i++) {
    ...
    modifier neededVotesReached(uint256 _proposalId) {
        // This approach always rounds DOWN
        uint256 needed = getNeededVotes(_proposalId);
        uint256 votes = proposals[_proposalId].yea.length +
            proposals[_proposalId].nay.length;
